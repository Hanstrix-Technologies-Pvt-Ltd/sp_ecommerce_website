...
type HeroContent = Pick<RAndDContent, "title" | "content">;
...
export default function Hero({ content }: { content: HeroContent }): JSX.Element {
  const firstPart = content.title[0];
  const secondPart = content.title[1];
        ...
        <div className="mt-[15px] tablet:pb-10 flex flex-col items-start tablet:flex-row tablet:items-center gap-4 laptop:gap-6">
          ...
          <div className="order-1 w-full tablet:order-2 tablet:flex-1">
            ...
            <div className="mt-5 text-[#616161] leading-[1.7] text-[17px]">
              {content.content.para.map((p, idx) => (
                <p key={idx}>{p}</p>
              ))}
            </div>
          </div>
