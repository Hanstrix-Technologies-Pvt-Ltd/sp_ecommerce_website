...
import type { AboutSection } from "@/data/AboutContent";
...
export function AboutSectionBlock({ section }: { section: AboutSection }) {
  ...
  return (
    <section className="space-y-4 md:space-y-5">
      ...
      {heading ? (
        <h3 className="text-[18px] md:text-[20px] font-semibold text-[#0a1a33]">{heading}</h3>
      ) : null}
      ...
      {items?.length ? (
        <ul className="mt-2 space-y-2">
          {items.map((it, idx) => (
            <li key={idx} className="text-[15px] md:text-[16px] leading-7 text-neutral-700">
